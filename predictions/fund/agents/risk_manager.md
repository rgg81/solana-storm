...
3. Symbol meets ALL: **`optimist_consensus ≥ +0.30`** (the mean of MA-Opt + SE-Opt — the BULLS' agreement; this REPLACED the old pessimist-dragged 4-way `consensus ≥ +0.20` on 2026-06-16, desk-forensics wf_8321460b: the flat 4-way mean let the two structurally-bearish specialists, averaging ≈ −0.24, cap blended consensus near +0.167 even at max optimism, making the probe arithmetically unreachable in ~77% of rows — the dominant inaction root cause), `ma_optimist ≥ +0.45`, no MA-Pes HARD VETO (`ma_pessimist > -0.50` — the pessimist is now a VETO, not an averaging drag), `onchain_consensus ≥ +0.00`, `combined_uncertainty < 0.55`, `liq/mcap ≥ 0.5%`, no validated reflection-rule HARD VETO. **The pessimist pair still protects the trade via the HARD VETO + the uncertainty cap — this change does NOT weaken any veto; it only stops two bears from diluting a genuine bull case below the entry bar.** The full-size BUY floor (rule #3, `consensus ≥ +0.40`) is UNCHANGED — only the $125 probe gate uses `optimist_consensus`.
