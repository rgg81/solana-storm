...
def calibration_table(
    ...
    """Bin predicted positive-return probabilities against the observed rate.

    Each row: the bin midpoint, the mean predicted probability, the observed
    positive-return rate, and the bin's token count. Empty bins are dropped.
    """
...
def _plot_calibration(table, path) -> None:
    ...
    if len(table) > 0:
        plt.plot(table["predicted_mean"], table["observed_rate"],
                 "o-", label="model")
    plt.title("Positive-return probability calibration")
    plt.xlabel("predicted positive-return probability")
    plt.ylabel("observed positive-return rate")
    plt.legend()
    plt.tight_layout()
    plt.savefig(path, dpi=110)
    plt.close()
...
def _render_markdown(**ctx) -> str:
    ...
    lines: List[str] = []
    lines.append("# Price-Prediction Pivot — Backtest Report")
    lines.append("")
    lines.append(
        "An honest walk-forward, portfolio-evolution backtest of the "
        "pump.fun token positive-forward-return strategy. Generated by `model/run.py`."
    )
    lines.append("")
